...
m.html.Schema=function(q){
    ...
    if(q.verify_html===false){
        ...
        function s(H){
        var G,C,V,R,W,B,E,Q,T,M,U,Y,K,F,S,A,O,D,X,Z,L,P,J=/^([#+-])?([^\[\/]+)(?:\/([^\[]+))?(?:\[([^\]]+)\])?$/,N=/^([!\-])?(\w+::\w+|[^=:<]+)?(?:([=:<])(.*))?$/,I=/[*?+]/;
        if(H){
            H=i(H);
            if(n["@"]){
                O=n["@"].attributes;
                    ...
                    if(S==="#"){
                        ...
                        for(V=0,R=T.length;V<R;V++){
                            ...
                            if(B){
                                ...
                                P=B[4];
                                if(Y==="!"){
                                    E.attributesRequired=E.attributesRequired||[];
                                    E.attributesRequired.push(U);
                                    Q.required=true
                                    }
                                    if(Y==="-"){
                                    delete K[U];
                                    F.splice(m.inArray(F,U),1);
                                    continue
                                }
                                if(S){
                                    if(S==="="){
                                        E.attributesDefault=E.attributesDefault||[];
                                        E.attributesDefault.push({
                                            name:U,
                                            value:P
                                        });
                                        Q.defaultValue=P
                                        ...
                                        if(S===":"){
                                        E.attributesForced=E.attributesForced||[];
                                ...
                                if(I.test(U)){
                                E.attributePatterns=E.attributePatterns||[];
                                Q.pattern=y(U);
                                E.attributePatterns.push(Q)
                                }else{
                                if(!K[U]){
                                    F.push(U)
                                    }
...
t("+ol[ul|ol],+ul[ul|ol]");
if(!v("span")){
    s("span[!data-mce-type|*]")
    }
    if(q.invalid_elements){
    m.each(m.explode(q.invalid_elements),function(A){
        if(n[A]){
            delete n[A]
        }
    })
...
z.getElementRule=v;
z.getCustomElements=function(){
    return c
    };

z.addValidElements=s;
z.setValidElements=u;
z.addCustomElements=r;
z.addValidChildren=t
};
...
(function(a){
    ...
    b.parse=function(q){
        var B=this,f,m=0,H,j,l=[],C,M,u,P,s,K,G,k,p,y,J,r,F,o,L,n,I,O,N,A,E,h,g,v,t=0,x=a.html.Entities.decode,z;
        function D(Q){
            var S,R;
            S=l.length;
            while(S--){
...
E=new RegExp("<(?:(?:!--([\\w\\W]*?)-->)|(?:!\\[CDATA\\[([\\w\\W]*?)\\]\\]>)|(?:!DOCTYPE([\\w\\W]*?)>)|(?:\\?([^\\s\\/<>]+) ?([\\w\\W]*?)[?/]>)|(?:\\/([^>]+)>)|(?:([^\\s\\/<>]+)\\s*((?:[^\"'>]+(?:(?:\"[^\"]*\")|(?:'[^']*')|[^>]*))*)>))","g");
    h=/([\w:\-]+)(?:\s*=\s*(?:(?:\"((?:\\.|[^\"])*)\")|(?:\'((?:\\.|[^\'])*)\')|([^>\s]+)))?/g;
    g={
    script:/<\/script[^>]*>/gi,
    style:/<\/style[^>]*>/gi,
...
G=e.getShortEndedElements();
    A=e.getSelfClosingElements();
    k=e.getBoolAttrs();
    y=c.validate;
    K=c.remove_internals;
    z=c.fix_self_closing;
    while(f=E.exec(q)){
    if(m<f.index){
        B.text(x(q.substr(m,f.index-m)))
        }
        if(H=f[6]){
        D(H.toLowerCase())
        }else{
        if(H=f[7]){
            H=H.toLowerCase();
            p=H in G;
            if(z&&A[H]&&l.length>0&&l[l.length-1].name===H){
                D(H)
                }
                if(!y||(J=e.getElementRule(H))){
                r=true;
                if(y){
                    L=J.attributes;
                    n=J.attributePatterns
                    }
                    if(o=f[8]){
                    s=o.indexOf("data-mce-type")!==-1;
                    if(s&&K){
                        r=false
                        }
                        C=[];
                    ...
                    o.replace(h,function(R,Q,V,U,T){
                        var W,S;
                        Q=Q.toLowerCase();
                        V=Q in k?Q:x(V||U||T||"");
                        if(y&&!s&&Q.indexOf("data-")!==0){
                            W=L[Q];
                            if(!W&&n){
                                S=n.length;
                                while(S--){
                    ...
                    if(W.validValues&&!(V in W.validValues)){
                        return
                    }
                    }
                    C.map[Q]=V;
    ...
    if(y&&!s){
    I=J.attributesRequired;
    O=J.attributesDefault;
    N=J.attributesForced;
    if(N){
        M=N.length;
        while(M--){
            F=N[M];
...
getAttrib:function(o,p,l){
    ...
    if(/^(checked|compact|declare|defer|disabled|ismap|multiple|nohref|noshade|nowrap|readonly|selected)$/.test(p)){
    ...
    j=j||o.style.cssText;
    if(j){
        j=k.serializeStyle(k.parseStyle(j),o.nodeName);
...
setupIframe:function(x){
    ...
    if((!b||!m.relaxedDomain)&&!x){
        if(a&&!v.readonly){
            q.getWin().addEventListener("DOMContentLoaded",function(){
...
q.quirks=new m.util.Quirks(this);
if(v.directionality){
...
focus:function(u){
    var y,q=this,s=q.selection,x=q.settings.content_editable,r,p,v=q.getDoc();
    if(!u){
        r=s.getRng();
        if(r.item){
            p=r.item(0)
        ...
        if(!x){
            ...
            if(p&&p.ownerDocument==v){
            r=v.body.createControlRange();
            r.addElement(p);
            r.select()
            }
        }
    if(m.activeEditor!=q){
    if((y=m.activeEditor)!=null){
        y.onDeactivate.dispatch(y,q)
        }
        q.onActivate.dispatch(q,y)
    }
    m._setActive(q)
},
...
B.onKeyDown.add(function(t,D){
    ...
    var F=t.selection.getRng().startContainer;
    var E=t.selection.getRng().startOffset;
...
if(m.isWebKit){
    q.bind(B.getDoc(),"selectionchange",function(){
        if(B.selectionTimer){
            window.clearTimeout(B.selectionTimer);
            B.selectionTimer=0
            }
...
function T(Z,ah,ac){
    var ad=R(Z),ai=ad[0],ag,X,af,ae=q.isCollapsed();
    function aa(am){
        var al=am.startContainer,ap=am.startOffset,ao,an;
        if(al.nodeType==1||al.nodeValue===""){
            al=al.nodeType==1?al.childNodes[ap]:al;
            if(al){
                ao=new t(al,al.parentNode);
...
O(ao.childNodes,function(ay,ax){
    if(ay.nodeName==="SPAN"&&c.getAttrib(ay,"data-mce-type")=="bookmark"&&ay.id==au.id+"_start"){
        aw=ax
        }
        if(ay.nodeName==="SPAN"&&c.getAttrib(ay,"data-mce-type")=="bookmark"&&ay.id==au.id+"_end"){
        ap=ax
        }
    });
if(an<=0||(aw<an&&ap>an)){
    O(a.grep(ao.childNodes),am);
    return 0
    }else{
    av=ar.cloneNode(S);
...
function aj(am,ao){
    var al=[],ap,an;
    ap=ai.inline||ai.block;
    an=c.create(ap);
    W(an);
    K.walk(am,function(aq){
        var ar;
        function at(au){
            var ax=au.nodeName.toLowerCase(),aw=au.parentNode.nodeName.toLowerCase(),av;
            if(g(ax,"br")){
                ar=0;
                if(ai.block){
                    c.remove(au)
            ...
            if(ai.wrapper&&x(au,Z,ah)){
                ar=0;
                return
            }
            if(ai.block&&!ai.wrapper&&G(ax)){
                au=c.rename(au,ap);
                W(au);
                al.push(au);
                ar=0;
            ...
            if(ai.selector){
                O(ad,function(ay){
                    if("collapsed" in ay&&ay.collapsed!==ae){
                        return
                    }
                    if(c.is(au,ay.selector)&&!b(au)){
                        W(au,ay);
                        av=true
                        }
                    });
        ...
        if(d(ap,ax)&&d(aw,ap)&&!(au.nodeType===3&&au.nodeValue.length===1&&au.nodeValue.charCodeAt(0)===65279)){
        if(!ar){
            ar=an.cloneNode(S);
...
if(ai){
    if(ac){
        ...
        if(!ae||!ai.inline||c.select("td.mceSelected,th.mceSelected").length){
            ...
            V.selection.setRng(ab());
            ag=q.getBookmark();
            aj(o(q.getRng(B),ad),ag);
            if(ai.styles&&(ai.styles.color||ai.styles.textDecoration)){
                a.walk(ak,I,"childNodes");
                I(ak)
                }
                q.moveToBookmark(ag);
            q.setRng(aa(q.getRng(B)));
            V.nodeChanged()
            }else{
            Q("apply",Z,ah)
            }
...
function G(W){
    ...
    function Q(ad,Y,ac){
    var aa,X=P[ad],ae=P[ad=="apply"?"remove":"apply"];
    function af(){
        ...
        O(P.apply.reverse(),function(ai){
            T(ai.name,ai.vars,ah);
            if(ai.name==="forecolor"&&ai.vars.value){
                I(ah.parentNode)
                }
            });
...
if(!P.isListening&&af()){
    ...
    O("onKeyDown,onKeyUp,onKeyPress,onMouseUp".split(","),function(ah){
        V[ah].addToTop(function(ai,al){
            if(al.keyCode==13&&!al.shiftKey){
                Z=true;
                return
            }
            if(af()&&!a.dom.RangeUtils.compareRanges(P.lastRng,q.getRng())){
                var aj=false;
                O(c.select("font,span"),function(ao){
                    var ap,an;
                    if(b(ao)){
                        aj=true;
                        ap=ao.firstChild;
                        while(ap&&ap.nodeType!=3){
                            ap=ap.firstChild
                            }
                            if(ap){
                            W(ao,ap)
        ...
        if(al.type=="keyup"||al.type=="mouseup"){
            ab();
            Z=false
            }
        }
